package net.braniumacademy.lesson57.exercises8;

public class BinarySearchTree<T extends Number> {
    private Node<T> root;

    public int removeNodes() {
        var targetNodes = firstRightNodeWithTwoChildren(root.rightNode);
        if(targetNodes != null) {
            remove(targetNodes);
        }
        return targetNodes != null ? 1 : 0;
    }

    private T firstRightNodeWithTwoChildren(Node<T> r) {
        if (r == null) {
        ...
        return firstRightNodeWithTwoChildren(r.rightNode);
    }
