package net.braniumacademy.lesson57.exercises7;

public class BinarySearchTree<T extends Number> {
    private Node<T> root;

    public int removeNodes() {
        var targetNodes = firstLeftNodeWithTwoChildren(root.leftNode);
        if(targetNodes != null) {
            remove(targetNodes);
        }
    ...
    private T firstLeftNodeWithTwoChildren(Node<T> r) {
        if (r == null) {
        ...
        return firstLeftNodeWithTwoChildren(r.leftNode);
    }
